...
# -- 2. RK4 Integrator --------------------------------------------------------
# -- Warp kernel: one timestep for all vehicles ------------------------------

@wp.kernel
def rk4_step_freeflight(states: wp.array2d(dtype=wp.float32), dt: float):
    """
    Advance all states by one timestep under free flight.

    states: (N, 6)  [x, y, z, vx, vy, vz]
    dt:     timestep

    For free flight, RK4 reduces to the exact solution:
        x_{t+dt} = x_t + v * dt
        v_{t+dt} = v_t
    """
    i = wp.tid()  # vehicle index
    ...
    # Position update; velocity is constant
    x_next = x + vx * dt
    y_next = y + vy * dt
    z_next = z + vz * dt
...
# Batched version using Warp kernel: vectorize simulate over N initial states -> (N, n_steps+1, 6)
def simulate_batch_warp(initial_states: torch.Tensor, t0: float, dt: float, n_steps: int):
    """
    Run simulations for a batch of initial states using a Warp kernel.

    Args:
        initial_states: (N, 6) torch float32 tensor on CUDA
        t0:             initial time (unused here)
        dt:             timestep
        n_steps:        number of integration steps

    Returns:
        trajectories: (N, n_steps + 1, 6) torch tensor on same device
    """
    assert initial_states.is_cuda, "simulate_batch_warp expects CUDA tensors"
    assert initial_states.dtype == torch.float32, "Warp example assumes float32"

    device = initial_states.device
    n_vehicles = initial_states.shape[0]

    # Pre‑allocate trajectories on the same torch device
    trajectories = torch.zeros(
        (n_vehicles, n_steps + 1, 6),
        device=device,
        dtype=initial_states.dtype,
    )
    trajectories[:, 0] = initial_states

    # Clone state so we can update in‑place
    states_torch = initial_states.clone()

    # Share the underlying CUDA buffer with Warp (no copy)
    states_wp = wp.from_torch(states_torch, dtype=wp.float32, requires_grad=False)

    # Warp device string, e.g. "cuda:0"
    if device.index is None:
        wp_device = "cuda"
    else:
        wp_device = f"cuda:{device.index}"

    t = t0  # kept for interface symmetry; unused in kernel

    for i in range(n_steps):
        # Launch one kernel over N vehicles
        wp.launch(
            rk4_step_freeflight,
            dim=n_vehicles,
            inputs=[states_wp, dt],
        ...
        # states_torch now contains updated states (shared memory)
        trajectories[:, i + 1] = states_torch
        t += dt
...
def main():
    ...
    parser.add_argument("--use-warp", action="store_true", help="Use Warp kernel for batched simulation (CUDA only)")
    ...
    if n_sims == 1:
        ...
        if args.use_warp and device.type == "cuda":
            trajectories = simulate_batch_warp(initial_states, t0, dt, n_steps)
            backend = "Warp"
        else:
            trajectories = simulate_batch(initial_states, t0, dt, n_steps)
            backend = "PyTorch"
